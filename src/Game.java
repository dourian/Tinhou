...
public class Game {
	...
	Game(int h, int w, boolean mouse, String file) {
		...
		try {
			backgroundImage = ImageIO.read(getClass().getResourceAsStream("resources/background_scaled_1600_900.png"));
	...
	public synchronized void repaint(Graphics g) {
		if(!list.contains(player)) {
			try {
				if(Math.random() < 0.99) {
					g.drawImage(ImageIO.read(getClass().getResourceAsStream("resources/losescreen_v2.png")), 0, 0, null);
					g.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 50));
					g.setColor(Color.WHITE);
					g.drawString(Long.toString(getScore()), 511, 367);
				}
				else {
					g.drawImage(ImageIO.read(getClass().getResourceAsStream("resources/losescreen.png")), 0, 0, null);
				}
			} catch (IOException e) {e.printStackTrace();}
			return;
		}
		...
		if(sp.audioPos()+window < sp.sze()) {
			...
			try {
				if(Math.random() < 0.99) {
					g.drawImage(ImageIO.read(getClass().getResourceAsStream("resources/winscreen_v2.png")), 0, 0, null);
					g.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 50));
					g.setColor(Color.WHITE);
					g.drawString(Long.toString(getScore()), 511, 367);
				}
				else {
					g.drawImage(ImageIO.read(getClass().getResourceAsStream("resources/winscreen.png")), 0, 0, null);
				}
			} catch (IOException e) {e.printStackTrace();}
		}
	}
